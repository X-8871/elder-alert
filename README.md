...
- MAX98357A I2S 功放播放固定中文提示音；动态 AI/TTS 回复播放代码已接入但仍待真实云端部署和实机验收
...
- `GET /api/speech/reply-audio` 可按最新 AI 回复生成语音音频，需配置真实 TTS 环境变量后使用
- 网页监控面板展示设备状态、风险原因、历史事件、语音识别结果和 AI Reply 区域
...
- **更自然的交互**：继续完善 TTS 和按键式语音对话，让设备能主动解释风险和回应老人；当前动态回复播放仍需真实云端部署和实机验收。
