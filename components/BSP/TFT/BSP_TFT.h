 ...
 *
 * 【学弟必读：ST7735 是什么？】
 * ST7735 是一个 SPI 接口的彩色 TFT 液晶控制器，常用于 1.8 寸小屏幕。
 * 本项目用它作为主显示屏，配合 LVGL 库渲染图形界面。
 *
 * 【为什么代码里用 st7789 驱动？】
 * ST7735 和 ST7789 寄存器兼容度很高（指令集几乎相同），
 * ESP-IDF 没有专门的 st7735 驱动，但有 st7789 驱动。
 * 我们用 st7789 驱动 + ST7735 专用初始化序列 + gap/mirror/swap_xy 参数补偿，
 * 就能让 ST7735 正常工作。这种"兼容驱动"技巧在嵌入式开发中很常见。
 *
 * 【关键参数】
 * - 逻辑分辨率：横屏 160×128（swap_xy=true 把物理 128×160 旋转90°）
 * - SPI 时钟：10MHz
 * - 颜色格式：RGB565（16-bit = 红5位 + 绿6位 + 蓝5位）
 */
