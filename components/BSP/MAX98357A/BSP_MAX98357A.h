 ...
 *
 * 【学弟必读：MAX98357A 是什么？】
 * MAX98357A 是一个 I2S 输入的 D 类音频功放芯片，可以直接驱动小喇叭（3W 以下）。
 * - I2S 数字音频输入，不需要 DAC 转换
 * - 单声道输出（本项目只接一个喇叭）
 * - SD (Shutdown) 引脚用于关断功放省电/消除杂音
 *
 * 【D 类功放的优点】
 * 效率高（>90%），发热小，适合电池供电的嵌入式设备。
 *
 * 【半双工共用 I2S】
 * INMP441(麦克风) 和 MAX98357A(功放) 共用 BCLK 和 WS 线，
 * 分别用不同的数据线（DIN 和 DOUT）。
 * 虽然 I2S 协议支持全双工，但当前实现为半双工：
 * 录音时释放功放，播放时释放麦克风，避免引脚冲突。
 */
